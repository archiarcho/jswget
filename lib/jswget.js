...
function parseConfig(argv){
	...
	configs.downloadas = (configs.downloadas===undefined)? ( (configs.downloadmode)? configs.path.split("/")[configs.path.split("/").length - 1]:configs.downloadas ):configs.downloadas; //nzo, replaced final "undefined" with configs.downloadas, to set the file name correctly.
...
function createRequest(protocol, sendoptions, configs){	
	...
	request.on('response', function(response) {	
		...
		if (configs.downloadmode && response.statusCode < 400) { //nzo, to prevent empty file creation if 404 error occures for exemple
			// check if is ready
			if (configs.downstream === undefined) {
				...
				response.on('end', function(){
					...
					var startbyte;
					try {
						fstat = Fs.statSync(configs.downloadpath /* nzo, the path contains the file name already + configs.downloadas */);
					} catch (e) {}
					if (fstat == null) {
						startbyte = 0;
					} else {
					...
					var file = Fs.createWriteStream(configs.downloadpath /* nzo, + configs.downloadas */, {'flags': 'a+'});
